...
# --- 2. CONNESSIONE DATABASE (una sola, condivisa) ---
@st.cache_resource
def get_engine():
...
if giorno_sel == "Giorno 4":
    ...
    try:
        with engine.connect() as conn:
            df_corsa_db = pd.read_sql("SELECT carico_kg FROM sessioni_allenamento WHERE esercizio = 'Corsa'", conn)
    ...
    if is_admin:
        if st.button("SALVA SESSIONE CORSA"):
            with engine.connect() as conn:
                ...
                """), {"d": data_sel, "g": giorno_sel, "r": minuti, "c": distanza, "n": note_corsa_finale})
                conn.commit()
            st.rerun()
    else:
    ...
    try:
        with engine.connect() as conn:
            df_corse = pd.read_sql("""
                SELECT id, data_allenamento AS Data, carico_kg AS Km,
                       ripetizioni AS Minuti, note AS Note
